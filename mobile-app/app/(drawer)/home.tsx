...
export default function HomeScreen() {
  ...
  const fetchUserBookings = useCallback(async () => {
    try {
      ...
      setBookedServices(
        res.data.map((b: any) => b.service_id)
      );
    } catch (err: any) {
      toast.error(getErrorMessage(err));
    }
  ...
  useEffect(() => {
    fetchServices();
    fetchUserBookings();
  }, [fetchServices, fetchUserBookings]);
  ...
  const detectCategory = useCallback(
    (title: string, desc: string): ServiceCategoryId => {
          ...
          ListEmptyComponent={
            ...
            selectedCategory !== "all" ? (
              ...
              <EmptyState
                ...
                subtitle="Services will appear here once workers add them"
              />
            )
          }
          renderItem={({ item }: any) => {
            const isBooked =
              bookedServices.includes(item.id);

            const wName =
              item.worker_name || "Worker";

            const avColor =
              getAvatarColor(wName);
